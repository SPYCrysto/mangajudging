...
export function ComicCard({ children, className = '', hoverEffect = false, ...props }: ComicCardProps) {
  return (
    <motion.div
      className={`comic-panel p-6 bg-white ${className}`}
...
export function ComicButton({
  ...
  const sizes = {
    ...
    md: "px-8 py-3 text-3xl",
    lg: "px-12 py-5 text-5xl",
  };
...
export function SectionTitle({ children, className = '' }: { children: React.ReactNode, className?: string }) {
  return (
    <div className={`relative inline-block ${className}`}>
      ...
      <h2 className="relative z-10 text-white text-4xl md:text-5xl font-display px-4 py-1">{children}</h2>
    </div>
  );
}
